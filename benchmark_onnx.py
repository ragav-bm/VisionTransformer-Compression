...
def benchmark_onnx_model(onnx_path, num_warmup=50, num_runs=200, providers=None):
    """
    Measures ONNX Runtime execution latency (p50, p95, p99) and throughput (FPS).
    """
    ...
    # Batch size 1 for real-time edge streaming
    batch_size = 1
    c = 3
    ...
    # 1. Warmup runs (stabilizes CPU clock governors and runtime caches)
    for _ in range(num_warmup):
        _ = session.run(None, {input_name: dummy_input})

    # 2. Timed runs using high-resolution monotonic timer
    latencies = []
...
def parse_args():
    parser = argparse.ArgumentParser(description="ONNX Runtime Edge Profiling & Latency Harness")
...
def main():
    ...
    print("=" * 75)
    print("⚡ ONNX RUNTIME EDGE LATENCY & THROUGHPUT HARNESS")
    print(f"   Execution Providers : {providers}")
    print(f"   Iterations          : {args.runs} timed runs ({args.warmup} warmup)")
    print(f"   Inference Batch Size: 1 (Edge Streaming)")
    print("=" * 75)
    ...
    for display_name, fname in models:
        ...
        if not os.path.exists(fpath):
            print(f"| {display_name:<20} | [Missing: {fname}] Run export_onnx.py & quantize.py first!")
            continue
